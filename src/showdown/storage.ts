...
const autoPlaySettings: AutoReplaySettings = new AutoReplaySettings(
  localStorage.getItem('auto_replay_settings'),
);
const updateSettings = () =>
  localStorage.setItem(
    'auto_replay_settings',
    JSON.stringify(autoPlaySettings),
  );
updateOldSettings();
...
function updateOldSettings(): void {
  const active = localStorage.getItem('auto_replay_active');
  const notifications = localStorage.getItem('auto_replay_notifications');
  const vgc_only = localStorage.getItem('auto_replay_vgc_only');
